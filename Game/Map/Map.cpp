...
Map::Map() {
    ...
    colorWall = sf::Color(63, 81, 181);
    colorSpace = sf::Color(0, 0, 0);
    colorExit = sf::Color(255, 245, 157);
...
void Map::createCellMap() {
    for (int i = 0; i < Map::HEIGHT_MAP; i++) {
        for (int j = 0; j < Map::WEIGHT_MAP; j++) {
            ...
            if (map[i][j] == '1') {
                /* init cells */
                cellMap[i][j] = Cell(Point(j * 24, i * 24), Point(j * 24 + 24, i * 24 + 24), IMPASSABLE);
            }

            // бонус
            if (map[i][j] == '0' || map[i][j] == ' ' || map[i][j] == '.') {
                /* init cells */
                cellMap[i][j] = Cell(Point(j * 24, i * 24), Point(j * 24 + 24, i * 24 + 24), SUPERFOOD);
            }

            // дверь
            if (map[i][j] == 'e') {
                /* init cells */
                cellMap[i][j] = Cell(Point(j * 24, i * 24), Point(j * 24 + 24, i * 24 + 24), DOOR);
            }

            // пустота
            if (map[i][j] == ' ') {
                /* init cells */
                cellMap[i][j] = Cell(Point(j * 24, i * 24), Point(j * 24 + 24, i * 24 + 24), PASSABLE);
            }

            // еда
            if (map[i][j] == '.') {
                /* init cells */
                cellMap[i][j] = Cell(Point(j * 24, i * 24), Point(j * 24 + 24, i * 24 + 24), FOOD);
            }
        }
